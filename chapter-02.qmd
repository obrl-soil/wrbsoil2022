...
9.  On a **slope**, the soil is described as a vertical profile. The thickness and depth values are calculated by multiplying the vertically measured values by the cosine of the inclination angle (see [Annex 1, Chapter 8.1.2](annex-01-field-guide.qmd#sec-a1-dig)) [@prietzel2019]. This is especially important on steep slopes.
...
Describe the soil applying the Field Guide in [Annex 1](annex-01-field-guide.qmd) (Chapter 8). It is useful that you already in the field compile a list of the possible diagnostic horizons, properties and materials observed (see [Chapter 3](chapter-03.qmd)). Conduct the relevant analyses according to [Annex 2](annex-02-analytical-procedures.qmd) (Chapter 9). Then, decide on the presence of diagnostics. **For the decision, only the diagnostic criteria are relevant** - neither the name of the diagnostic, nor any other description. A layer may fulfil the criteria of more than one diagnostic horizon, property or material, which are then regarded as overlapping or coinciding.
...
|-----------------|--------------------------------------|-----------------|
| b\. | [*argic horizon*](chapter-03.qmd#sec-argich) with high CEC and more base cations than Al (inferred by pH 6)                | --\> [Luvisol](chapter-04.qmd#sec-key-lv)                            |
| c\. | light colour in the eluvial horizon                                                                                        | --\> [Albic](chapter-05.qmd#sec-ab) qualifier                        |
| d\. | some oximorphic features inside aggregates                                                                                 | --\> [*stagnic properties*](chapter-03.qmd#sec-stagnp)               |
| e\. | [*stagnic properties*](chapter-03.qmd#sec-stagnp) and [*reducing conditions*](chapter-03.qmd#sec-reducc) starting at 60 cm | --\> [Endo](#sec-endo)[stagnic](chapter-03.qmd#sec-stagnp) qualifier |
| f\. | clay coatings                                                                                                              | --\> [Cutanic](chapter-05.qmd#sec-ct) qualifier                      |
| g\. | clay increase without [*lithic discontinuity*](chapter-03.qmd#sec-lithd)                                                   | --\> [Differentic](chapter-05.qmd#sec-df) qualifier                  |

The **field classification** is Albic Endostagnic Luvisol (Cutanic, Differentic).

#### Laboratory analyses {#sec-nmex-ans}

The laboratory analyses confirm a high CEC kg^-1^ clay in the [*argic horizon*](chapter-03.qmd#sec-argich) and a high base saturation in the depth from 50 - 100 cm. They further detect the texture class of silty clay loam with 30% clay ([Siltic](chapter-05.qmd#sec-sl) qualifier) from 0 - 60 cm (Ano- specifier) and of silty clay with 45% clay ([Clayic](chapter-05.qmd#sec-ce) qualifier) from 60 - 100 cm (Endo- specifier). The organic carbon content in the topsoil is intermediate ([Ochric](chapter-05.qmd#sec-oh) qualifier).
...
1.  If a qualifier refers to a characteristic that occurs at a specific point of depth (e.g. Raptic), **optional subqualifiers** can be constructed with the following specifiers:
    ...
    -   [**Endo-**]{#sec-endo} (from Greek *endon*, inside): the characteristic is present somewhere \> 50 and ≤ 100 cm of the (mineral) soil surface and is absent ≤ 50 cm of the (mineral) soil surface.
...
2.  If a qualifier refers to a **layer** (e.g. [Calcic](chapter-05.qmd#sec-cc), [Arenic](chapter-05.qmd#sec-ar), [Fluvic](chapter-05.qmd#sec-fv)), **optional subqualifiers** can be constructed with the following specifiers (see Figure 2.1):
    ...
    -   **Endo-** (from Greek *endon*, inside): the layer starts ≥ 50 cm from the (mineral) soil surface; and no such layer occurs \< 50 cm of the (mineral) soil surface. (Examples: Endocalcic: the [*calcic horizon*](chapter-03.qmd#sec-calcich) starts ≥ 50 and ≤ 100 cm from the soil surface; Endospodic: the [*spodic horizon*](chapter-03.qmd#sec-spodich) starts ≥ 50 and ≤ 200 cm from the mineral soil surface.)
...
A landscape usually shows a variety of soils. For a map unit, they often have to be combined. The principles are shown in @fig-two-mapunits and in @tbl-cdsoils and @tbl-mapscales.
...
: Detection of dominant, codominant and associated soils {#tbl-cdsoils}
...
: Denomination of the map unit depending on the scale level {#tbl-mapscales}
