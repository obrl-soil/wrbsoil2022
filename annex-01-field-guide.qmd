...
tbl_a1827 <- gt(tbl_a1827_dat) |>
  ...
  text_transform(
    locations = cells_body(
      columns = c(Criterion), rows = c(3,4)
    ),
    fn = function(x){
      text <- str_extract(x, "[^-1^]+")
      glue::glue("{text}<sup>-1</sup>")
    }
  )
tbl_a1827
```
...
: Types of structure, descriptions, Schoeneberger et al, (2012), 2-53, FAO (2006), Table 49, National Committee on Soil and Terrain (2009), 171-181, modified {#tbl-a1841} <!--# TODO this needs to be tableified properly once I figure out how to do images and merged rows with gt -->
...
tbl_a1847 <- gt(tbl_a1847_dat) |>
  ...
  cols_label(Diam = 'Diameter (mm)',
             ...
             Size = 'Size Class') |>
  # https://stackoverflow.com/questions/69239911/subscripts-and-superscripts-in-body-of-gt-table-r
   text_transform(
    locations = cells_body(
      columns = c(Area)
    ),
    fn = function(x){
      text <- str_extract(x, "[^2^]+")
      glue::glue("{text}<sup>2</sup>")
    }
  )
...
tbl_a1855 <- gt(tbl_a1855_dat) |>
  ...
  tab_spanner(label = 'Location',
              columns = c(Loc_01, Loc_02),
              replace = FALSE, id = 'span1') |>
  tab_spanner(label = 'Code',
              columns = c(Code),
              replace = FALSE, id = 'span2') |>
