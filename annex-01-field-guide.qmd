...
'Formerly water-saturated for ≥ 30 consecutive days, then drained and now water-saturated for < 30 consecutive days', 'DR',
...
```{r}
#| label: tbl-a1828
#| tbl-cap: Soil water status, FAO [-@un-fao2006], Table 57, modified 
#| classes: no-stripe 
...
tbl_a1828 <- gt(tbl_a1828_dat) |>
  ...
  cols_width(ends_with("ing") ~ pct(30), Code ~ pct(20))
...
```{r}
#| label: tbl-a1830
#| tbl-cap: Distinctness of layer boundaries, Schoeneberger et al. [-@schoeneberger2012], 2-6, modified
#| classes: no-stripe
...
tbl_a1830 <- gt(tbl_a1830_dat) |>
  ...
  cols_width(ends_with('lyrs') ~ pct(27), Code ~ pct(20))
...
```{r}
#| label: tbl-a1831
#| tbl-cap: Shape of layer boundaries, Schoeneberger et al. [-@schoeneberger2012], 2-7
#| classes: no-stripe
...
tbl_a1831 <- gt(tbl_a1831_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Criterion ~ pct(50), Shape ~ pct(30), Code ~ pct(20))
...
```{r}
#| label: tbl-a1833
#| tbl-cap: Size and shape classes of coarse fragments and of remnants of broken-up cemented layers, FAO [-@un-fao2006], Tables 27 and 28
#| classes: no-stripe 
...
NA_character_, 'None',               NA_character_, 'NO')
...
tbl_a1833 <- gt(tbl_a1833_dat) |>
  ...
  cols_label(Size = 'Size (cm)',
             S_class = 'Size Class') |>
  sub_missing(missing_text = '') |>
  cols_width(Size ~ pct(20), S_class ~ pct(30), Shape ~ pct(30), 
             Code ~ pct(20))
...
```{r}
#| label: tbl-a1834
#| tbl-cap: Weathering stage of coarse fragments, FAO [-@un-fao2006], Table 29 
#| classes: no-stripe
...
tbl_a1834 <- gt(tbl_a1834_dat) |>
  ...
  cols_width(Criterion ~ pct(60), W_stage ~ pct(20))
...
```{r}
#| label: tbl-a1835
#| tbl-cap: "Remnants of broken-up cemented layers: cementing agent" 
#| classes: no-stripe
...
```{r}
#| label: tbl-a1836
#| tbl-cap: Examples of artefacts, Schoeneberger et al. [-@schoeneberger2012], 2-50, modified
#| classes: no-stripe
...
```{r}
#| label: tbl-a1837
#| tbl-cap: Size of artefacts, FAO [-@un-fao2006], Table 27
#| classes: no-stripe
...
tbl_a1837 <- gt(tbl_a1837_dat) |>
  ...
  cols_label(Size = 'Size (cm)',
             S_class = 'Size class') |>
  cols_width(starts_with('S') ~ pct(40))
...
```{r}
#| label: tbl-a1838
#| tbl-cap: Particle-size classes, ISO 11277:2009 [@internationalorganizationforstandardization2015]
#| classes: no-stripe
...
```{r}
#| label: tbl-a1839
#| tbl-cap: Texture classes, Soil Science Division Staff [-@soilsciencedivisionstaff2017]
#| classes: no-stripe
...
![Texture classes, flow chart, ideas adapted from Natural England Technical Information Note TIN037 [-@naturalengland2008] and Thien [-@thien1979]](images/figure_8-14_2024.png){#fig-a1-814}
...
```{r}
#| label: tbl-a1840
#| tbl-cap: Subclasses of the texture classes sand and loamy sand, Soil Science Division Staff [-@soilsciencedivisionstaff2017], modified; the percentages of the sand fractions are related to the entire fine earth (not related to sand).
#| classes: no-stripe
...
```{r}
#| label: tbl-a1841
#| tbl-cap: Types of structure, descriptions, Schoeneberger et al, [-@schoeneberger2012], 2-53, FAO [-@un-fao2006], Table 49, National Committee on Soil and Terrain [-@nationalcommitteeonsoilandterrain2009], 171-181, modified
#| classes: no-stripe  
...
tbl_a1842 <- gt(tbl_a1842_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Type ~ pct(20), Formation ~ pct(60))
...
```{r}
#| label: tbl-a1843
#| tbl-cap: Grade of structural units, Soil Science Division Staff [-@soilsciencedivisionstaff2017], 159f, modified
#| classes: no-stripe
...
tbl_a1843 <- gt(tbl_a1843_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Criterion ~ pct(70), Grade ~ pct(10))
...
```{r}
#| label: tbl-a1845
#| tbl-cap: Aggregate size, Schoeneberger et al. [-@schoeneberger2012], 2-55, FAO [-@un-fao2006], Table 50, modified
#| classes: no-stripe
...
```{r}
#| label: tbl-a1846
#| tbl-cap: Types of non-matrix pores, Schoeneberger et al. [-@schoeneberger2012], 2-73, modified
#| classes: no-stripe 
...
tbl_a1846 <- gt(tbl_a1846_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  sub_missing(missing_text = '') |>
  cols_width(Criterion ~ pct(60), Type ~ pct(20))
...
```{r}
#| label: tbl-a1847
#| tbl-cap: Pore size, Schoeneberger et al. [-@schoeneberger2012], 2-70
#| classes: no-stripe 
...
tbl_a1847 <- gt(tbl_a1847_dat) |>
  ...
  cols_width(Diam ~ pct(27), Area ~ pct(27), Size ~ pct(26))
...
```{r}
#| label: tbl-a1848
#| tbl-cap: Abundance of pores, Schoeneberger et al. [-@schoeneberger2012], 2-70, modified
#| classes: no-stripe
...
tbl_a1848 <- gt(tbl_a1848_dat) |>
  ...
  cols_width(Number ~ pct(20), Abundance ~ pct(60))
...
```{r}
#| label: tbl-a1849
#| tbl-cap: Persistence of cracks, Schoeneberger et al. [-@schoeneberger2012], 2-76
#| classes: no-stripe
...
```{r}
#| label: tbl-a1851
#| tbl-cap: Types of concentrations (overview), Soil Science Division Staff. [-@soilsciencedivisionstaff2017], page 174f
#| classes: no-stripe
...
'Covering the surfaces of coarse fragments, remnants of a broken-up cemented layers, aggregates or pore walls', 'Coating')
...
```{r}
#| label: tbl-a1852
#| tbl-cap: Size of lithogenic variegates, FAO [-@un-fao2006], Table 33
#| classes: no-stripe
...
tbl_a1852 <- gt(tbl_a1852_dat) |>
  ...
  cols_label(Size = 'Size (mm)',
             Class = 'Size class') |>
  cols_width(Size ~ pct(40), Class ~ pct(40))
...
Redoximorphic features (oximorphic features plus reductimorphic features) are the result of reduction processes or of reduction and subsequent re-oxidation processes. Oximorphic features show the accumulation of substances in oxidized state (concentrations) and usually have a redder hue, a higher chroma and a lower value than the surrounding material, while reductimorphic features show the opposite characteristics. Soil parts showing reductimorphic features may either contain substances in reduced state or may have lost them.
...
'', 'Inside soil aggregates: nodules', 'OIN',
...
tbl_a1855 <- gt(tbl_a1855_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_label(Loc_02 = '') |>
  cols_width(Location ~ pct(20), Loc_02 ~ pct(60))
  # n2s row merges on col 1 when possible
...
```{r}
#| label: tbl-a1857
#| tbl-cap: Size of oximorphic features, FAO [-@un-fao2006], Table 33
#| classes: no-stripe
...
```{r}
#| label: tbl-a1858
#| tbl-cap: Consistence of oximorphic features, Schoeneberger et al. [-@schoeneberger2012], 2-63
#| classes: no-stripe
...
tbl_a1858 <- gt(tbl_a1858_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Criterion ~ pct(50), Class ~ pct(30))
...
```{r}
#| label: tbl-a1859
#| tbl-cap: Ranges of rH values and related soil processes as derived from redoximorphic features and from field tests of reducing conditions, Blume et al. [-@blume2010], page 24, FAO [-@un-fao2006], Table 36, modified
#| classes: no-stripe
...
tbl_a1859 <- gt(tbl_a1859_dat) |>
  ...
  cols_width(Criterion ~ pct(40), Processes ~ pct(20), rH_value ~ pct(20))
...
The term 'oxides', as used here, includes hydroxides and oxide-hydroxides. If clay minerals are accumulated, a ribbon-like accumulation is \< 7.5 cm thick, in all other cases \< 2.5 cm. If there are 2 or more ribbon-like accumulations in one layer, report the number of the accumulations and their combined thickness in cm.

If clay minerals are accumulated (CC, CO, CH, TO), the ribbon-like accumulations are called **lamellae**. For lamellae, report additionally the texture class, the abundance of clay coatings and clay bridges and the combined thickness within 50 cm of the upper limit of the uppermost lamella.
...
```{r}
#| label: tbl-a1862
#| tbl-cap: Carbonate contents, FAO [-@un-fao2006], Table 38
#| classes: no-stripe
...
tbl_a1862 <- gt(tbl_a1862_dat) |>
  ...
  cols_width(Criterion ~ pct(40), Content ~ pct(20), pc_by_m ~ pct(20))
...
tbl_a1864_dat <- tribble(
  ~Type,   ~Code,
'Masses (including spheroidal aggregations like white eyes (byeloglaska); including masses filling the complete fine earth)',                  'MA',
...
'Coatings on undersides of coarse fragments and of remnants of broken-up cemented layers (with or without coatings on other sides)',                   'UR',
...
```{r}
#| label: tbl-a1866
#| tbl-cap: Gypsum contents in layers with little readily soluble salts, FAO [-@un-fao2006], Table 40 
#| classes: no-stripe 
...
tbl_a1866 <- gt(tbl_a1866_dat) |>
  ...
  text_transform(
    ...
    fn = \(x){ str_replace(x, "\\\n", '<br />')}) |>
  cols_width(EC ~ pct(40), Content ~ pct(20), pc_by_m ~ pct(20))
...
'Remnants of a layer that has been cemented by secondary silica', 'FC',
...
```{r}
#| label: tbl-a1868
#| tbl-cap: Size of durinodes and remnants of a layer that has been cemented by secondary silica
#| classes: no-stripe
...
tbl_a1869 <- gt(tbl_a1869_dat) |>
  ...
  tab_spanner(columns = c(c1,c2,c3,c4,c5,c6),
              ...
              level = 1, id = 'span1') |>
  cols_width(starts_with('c') ~ pct(12))
...
```{r}
#| label: tbl-a1870
#| tbl-cap:  Estimation of the water content of the saturation extract of organic layers, DVWK [-@bodenkun1995], FAO [-@un-fao2006], Table 43
#| classes: no-stripe
...
tbl_a1870 <- gt(tbl_a1870_dat) |>
  ...
  cols_label(decomp = html('Degree of decomposition<br />(by volume, related to the fine earth plus all dead plant residues)'),
             water_c = html('Water content of the saturation extract (WC<sub>SE</sub>) (g water / 100 g soil)')) |>
  cols_width(water_c ~ pct(30))
...
tbl_a1871 <- gt(tbl_a1871_dat) |>
  ...
  text_transform(
    ...
    fn = \(x) { str_replace(x, '~2~', '<sub>2</sub>') }) |>
  cols_width(Solution ~ pct(40), m_ratio ~ pct(40)) |>
  # not sure why tabl.align is being overridden on col 2, but
  cols_align(columns = m_ratio, align = 'left')
...
```{r}
#| label: tbl-a1872
#| tbl-cap: Cementing agents, Schoeneberger et al. [-@schoeneberger2012], 2-64
#| classes: no-stripe
...
```{r}
#| label: tbl-a1873
#| tbl-cap: Cementation, Schoeneberger et al. [-@schoeneberger2012], 2-63
#| classes: no-stripe  
...
'low of < 3 J (3 J = 2 kg dropped 15 cm)', 'Extremely strongly cemented', 'EXC')
...
```{r}
#| label: tbl-a1874
#| tbl-cap: Rupture resistance, non-cemented soil, Schoeneberger et al. [-@schoeneberger2012], 2-63 
#| classes: no-stripe 
...
'Slight force between fingers, 8 - < 20 N',  'Friable', 'FR', 'Slightly hard', 'SH',
...
tbl_a1874 <- gt(tbl_a1874_dat) |>
  ...
  cols_label(Class_m = 'Class',
             ...
             Code_m = 'Code',
             Code_d = 'Code') |>
  cols_width(starts_with('Class') ~ pct(15), 
             starts_with('Code') ~ pct(15))
...
```{r}
#| label: tbl-a1876
#| tbl-cap: Types of manner of failure (brittleness), Schoeneberger et al. [-@schoeneberger2012], 2-65
#| classes: no-stripe
...
```{r}
#| label: tbl-a1877
#| tbl-cap: Types of plasticity, Schoeneberger et al. [-@schoeneberger2012], 2-66
#| classes: no-stripe 
...
tbl_a1879 <- gt(tbl_a1879_dat) |>
  ...
  cols_width(Particles ~ pct(40), Code ~ pct(20))
...
tbl_a1883 <- gt(tbl_a1883_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Criterion ~ pct(60), Class ~ pct(20))
...
### Soil organic carbon (C~org~) {#sec-a1-8436}

#### Estimation of the content (m) (\*) {#sec-a1-8436-e}
...
```{r}
#| label: tbl-a1884
#| tbl-cap: Estimation of organic carbon contents in a moist sample, Blume et al. [-@blume2010], modified
#| classes: no-stripe
...
#### Natural accumulations of organic matter (m) {#sec-a1-8436-n}
...
Report up to three types, the dominant one first, and report the percentage (by exposed area) for each type separately.

#### Black carbon (o, m) {#sec-a1-8436-b}

Black carbon has to be additionally reported as percentage of the exposed area (related to the fine earth plus black carbon of any size).
...
```{r}
#| label: tbl-a1886
#| tbl-cap: Abundance of roots, FAO [-@un-fao2006], Table 80
#| classes: no-stripe
...
tbl_a1886 <- gt(tbl_a1886_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 4)) |>
  cols_width(lt2 ~ pct(27), lt2 ~ pct(27), Code ~ pct(20))
...
```{r}
#| label: tbl-a1887
#| tbl-cap: Types of animal activity, FAO [-@un-fao2006], Table 82, modified
#| classes: no-stripe
...
```{r}
#| label: tbl-a1891
#| tbl-cap:  Types of parent material, FAO [-@un-fao2006], Table 12, modified
#| classes: no-stripe
...
tbl_a1891 <- gt(tbl_a1891_dat) |>
  ...
  cols_width(Major_class ~ pct(25),
             Group ~ pct(25),
             starts_with('Code') ~ pct(10))
...
This Chapter refers to the transformation of recognizable dead plant tissues into visibly homogeneous organic matter. Rub the soil material and report the percentage of recognizable dead plant tissues (by volume, related to the fine earth plus all dead plant residues).
...
tbl_a1892 <- gt(tbl_a1892_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 3)) |>
  cols_width(Criterion ~ pct(60), Code ~ pct(20))
...
This Chapter refers to dead natural plant residues of any length and a diameter ≥ 5 mm. For processed plant residues, see [*artefacts*](chapter-03.qmd#sec-artefm) (see [Chapter 8.4.8](#sec-a1-848)). Report up to two types of plant remnants, the dominant one first, and give the percentage (by volume, related to the fine earth plus all dead plant residues) for each type separately.

```{r}
#| label: tbl-a1893
#| tbl-cap: Dead residues of specific plants
#| classes: no-stripe 
...
'Wood',                   'W',
'Moss fibres',            'S',
'No dead residues of wood or of moss fibres', 'N')
...
tbl_a1893 <- gt(tbl_a1893_dat) |>
  ...
  tab_style(style = list(cell_text(weight = 'bold')),
            locations = cells_body(columns = 2)) |> 
  cols_label(Type = 'Type of plant residues') |>
  cols_width(Code ~ pct(20))
